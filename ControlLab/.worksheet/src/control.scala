...
object control {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(127); 
  ...
  def tax(income: Double) =
  	income match{
  		...
  		case _ => .5 * income
  	};System.out.println("""tax: (income: Double)AnyVal""");$skip(116); val res$0 = 
  ...
  def pureBelow10(x: Option[Double]) =
  	x match{
  		...
  		case _ => None
  	};System.out.println("""pureBelow10: (x: Option[Double])Option[Double]""");$skip(152); 
  
  //use those to define: //don't use match here, combine two already defined methods
  def below10root(x: Option[Double]) = pureRoot(pureBelow10(x));System.out.println("""below10root: (x: Option[Double])Option[Double]""");$skip(21); val res$7 = 
